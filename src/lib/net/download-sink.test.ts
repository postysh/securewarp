...
describe("openDownloadSink", () => {
  ...
  describe("FSA path", () => {
    ...
    it("falls back to blob sink when FSA throws a non-AbortError (e.g. permission)", async () => {
  ...
  describe("Blob fallback", () => {
    it("returns a non-streaming sink when FSA is unavailable", async () => {
      installNoFsa();
      const sink = await openDownloadSink("f", "application/octet-stream");
      expect(sink.streaming).toBe(false);
    });
    ...
    it("close() assembles chunks, triggers an anchor click, and revokes the blob URL", async () => {
      installNoFsa();
      const clickSpy = vi.fn();
      const anchor = {
        set href(_v: string) {},
