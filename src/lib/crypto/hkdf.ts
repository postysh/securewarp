 ...
 *                            round-trip)
 *
 * All three keys are derived from the same master key via different
 * HKDF info strings, so knowing any one of them leaks nothing about
 * the others. Each has its own `-v1` versioning suffix so future
 * algorithm changes can coexist with existing data.
 */
